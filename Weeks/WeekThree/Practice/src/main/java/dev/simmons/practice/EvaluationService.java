...
public class EvaluationService {
    ...
    public String acronym(String phrase) {
        ...
        Arrays.stream(pieces).map(s -> Character.toUpperCase(s.charAt(0))).forEach(sb::append);
    ...
    public int getScrabbleScore(String string) {
        ...
        List<Character> chars = string.toLowerCase().chars().mapToObj(c -> (char)c).collect(Collectors.toList());
        return chars.stream().map(c -> scores[c-'a']).reduce(0, (s, c) -> s+c);
    }
    ...
    public String cleanPhoneNumber(String string) {
        String toStrip = " ()-.";
        string = string.replaceFirst("(\\+1\\s?)?(1\\s?)?", "");
        StringBuilder sb = new StringBuilder();
        List<Character> chars = string.chars().mapToObj(c -> (char)c)
                .filter(c -> !toStrip.contains(c+"")).collect(Collectors.toList());

        if (chars.stream().anyMatch(c -> !Character.isDigit(c))) {
    ...
    public String toPigLatin(String string) {
        ...
        Arrays.stream(string.split(" ")).map(s -> wordToPigLatin(s) + " ").forEach(sb::append);
    ...
    public boolean isArmstrongNumber(int input) {
        ...
        int candidate = string.chars().map(c -> (int)(Math.pow(c - '0', length))).reduce(0, (i,j) -> i+j);
    ...
    static class RotationalCipher {
        ...
        public String rotate(String string) {
            ...
            string.chars().mapToObj(c -> rotate((char)c)).forEach(sb::append);
    ...
    static class AtbashCipher {
        ...
        public static String encode(String string) {
            AtomicInteger spacing = new AtomicInteger();
            StringBuilder sb = new StringBuilder();

            List<Character> chars = string.toLowerCase().chars().mapToObj(c -> (char)c).filter(c -> Character.isLetterOrDigit(c)).collect(Collectors.toList());
            chars.stream().map(c -> {
                        if (Character.isDigit(c)) {
                            return c;
                        }
                        return (char) ('z' - c + 'a');
                    }).forEach(c -> sb.append((char)c).append(spacing.incrementAndGet() % 5 == 0 ? " " : ""));
        ...
        public static String decode(String string) {
            ...
            string.chars().filter(c -> c != ' ')
                    .mapToObj(c -> (char)(Character.isAlphabetic(c) ? ('z' - c + 'a') : c))
                    .forEach(sb::append);
    ...
    public boolean isValidIsbn(String string) {
        AtomicInteger mult = new AtomicInteger(10);
        int sum;

        IntStream chars = string.chars();
        if (chars.anyMatch(c -> Character.isAlphabetic(c) && c != 'X')) {
            return false;
        }
        sum = string.chars().filter(c -> c != '-').map(c -> {
            if (c == 'X') c = 10;
            else c -= '0';
            return c;
        }).reduce(0, (i,j) -> i+(j* mult.getAndDecrement()));
    ...
    public boolean isPangram(String string) {
        ...
        string.toLowerCase().chars().filter(Character::isAlphabetic).forEach(c -> chars.add((char)c));
